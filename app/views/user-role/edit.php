...
<form action="<?php echo getUrl( "user-role/save/{$USER_ROLE->id}", false, false, true ); ?>" method="post" enctype="multipart/form-data">
	<div class="card shadow">
	    <div class="card-header py-3 d-flex flex-row align-items-center justify-content-between pos-r">
	      	<h6 class="m-0 fw-b text-primary">Edit User Role</h6>
	    </div>
			...
			<div class="row">
		    	<div class="col">
			    	<div class="form-group row">
						...
						<small class="col-sm-4 d-flex align-items-center text-muted">Use only alphanumeric.</small>
					</div>
										...
										<dl>
											<?php foreach ( $permission_groups as $p_key => $permission ) { ?>
												<?php if ( !is_numeric( $p_key ) ) { ?>
													<dd>
														<div class="custom-control custom-checkbox">
														  	<input type="checkbox" name="permissions[]" value="<?php echo $p_key; ?>" <?php if ( array_key_exists( $p_key, $FORM['permissions'] ) ) echo 'checked'; ?> id="field-<?php echo $p_key; ?>" class="custom-control-input permissions">
